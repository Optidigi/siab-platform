import type { RtRoot, RtBlock, RtInline, RtMark, RtText, RtLink } from "../RtNode"
...
const inlineFromLexical = (n: any): RtInline | null => {
  ...
  if (n.type === "text" || n.type === "rt-text") {
    ...
    const marks = formatToMarks(n.format ?? 0)
...
const blockFromLexical = (n: any): RtBlock | null => {
  switch (n.type) {
    case "paragraph":
    ...
    case "horizontalrule":
      return { t: "divider" }
    case "themed": {
      const out: any = { t: "themed", id: n.id, props: n.props ?? {} }
...
export const lexicalJsonToRt = (j: { root: any }, variant: "block" | "inline"): RtRoot => {
  if (variant === "inline") {
