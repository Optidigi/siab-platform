...
describe("page editor explicit-save contract", () => {
  ...
  it("serializes media uploads and sparse theme state before explicit Save", () => {
    ...
    expect(source).toContain("blocks: normalizePageBlockUploadIds(values.blocks)")
    expect(source).toContain("ogImage: normalizeUploadId(values.seo.ogImage)")
  })
  ...
  it("keeps site chrome inspector changes local until explicit page Save", () => {
    ...
    expect(source).toContain("const pageWasDirty = !initial || form.formState.isDirty")
