...
describe("generation operations route access", () => {
  ...
  it("exposes Phase 8 snapshot lifecycle controls behind super-admin server actions", () => {
    ...
    const visiblePanel = detail.slice(detail.indexOf("<CardTitle>Send preview"), detail.indexOf("<CardTitle>Advanced"))
    ...
    expect(visiblePanel).toContain("<CardTitle>Send preview")
    expect(visiblePanel).toContain("<CardTitle>Client")
  ...
  it("presents the detail page as a manager-facing draft-site workflow", () => {
    ...
    expect(detail).toContain("<CardTitle>Client")
    expect(detail).toContain("<CardTitle>Status")
    expect(detail).toContain("Preview email")
    expect(detail.indexOf("<CardTitle>Send preview")).toBeLessThan(detail.indexOf("<CardTitle>Client"))
    expect(detail.indexOf("<CardTitle>Send preview")).toBeLessThan(detail.indexOf("<CardTitle>Advanced"))
