...
describe("mapHtmlToRt — declared color/style classes", () => {
  ...
  it("maps rt-type-lead on a span to text.style = lead", () => {
    ...
    expect((root.children[0] as any).children[1]).toMatchObject({ t: "text", v: "leady", style: "lead" })
  })
  ...
  it("maps rt-font-heading on a span to text.font = heading", () => {
    ...
    expect((root.children[0] as any).children[1]).toMatchObject({ t: "text", v: "serif", font: "heading" })
  })
  ...
  it("drops undeclared rt-color / rt-type / rt-font classes (no field set)", () => {
    ...
    expect((root.children[0] as any).children[0]).toEqual({ t: "text", v: "x" })
  })
