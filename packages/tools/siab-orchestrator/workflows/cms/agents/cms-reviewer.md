...
- **No unexpected dependency drift.** Compare `package.json` against the pre-conversion state — only the SSR/CMS runtime dependencies from Group 1 should have been added. The pre-conversion SHA is the parent of the first commit listed in the conversion report (typically `chore: install @astrojs/node + @astrojs/preact and switch to SSR output`):
  ```bash
  PRE_SHA=$(git merge-base HEAD origin/main)   # adjust if needed: should be origin/main pre-conversion
  git diff "$PRE_SHA"..HEAD -- package.json | grep -E '^\+\s+"' \
    | grep -Ev '"(@astrojs/node|@astrojs/preact|preact|lucide-preact)"' \
    && echo "BLOCKING: unexpected dependencies added beyond the Group 1 SSR/CMS runtime set" \
    || echo "OK: dependency additions are limited to the Group 1 SSR/CMS runtime set"
  ```
...
- `pnpm build` succeeds against the locked dependency tree:
  ```bash
  cd <site-repo> && pnpm install --frozen-lockfile && pnpm check:responsive && pnpm build
  ```
  Confirm both commands exit 0 and that `dist/server/entry.mjs` exists.
...
- `SiteSettings` matches the current Payload projection shape, not the older
  static `src/content/site.ts` shape. It must expose `siteName`, `siteUrl`,
  `contact.social`, `navHeader`, `navFooter`, `nap.streetAddress`, and
  `hours[].day/open/close`; it must not rely on `brand`, `primaryDomain`,
  `socials`, `nav`, `nap.street`, `hours[].dayOfWeek`, `opens`, or `closes`
  in CMS runtime files.
- Generic tenant fallbacks only. Runtime/layout/SEO/block files must not carry
  Amicare-specific fallback copy, domains, email addresses, anchors, service
  areas, or business identifiers:
  ```bash
  grep -RIn "Amicare\\|ami-care\\|Roermond\\|Limburg\\|Persoonlijke\\|werkwijze\\|wat-telt" src/layouts/ src/components/seo/ src/components/cms/ src/pages/ \
    && echo "BLOCKING: tenant-specific fallback leaked into generic CMS runtime" \
    || echo "OK: no tenant-specific runtime fallbacks"
  ```
...
grep -rEn "\.(blocks|items|features|fields|pills|navHeader|navFooter|social|aliases|hours|serviceArea)\??\.map\([^)]*\bid\b" src/
```
...
- **`src/layouts/BaseLayout.astro` injects tenant-theme.css**:
  ...
  ```
- **Maintenance banner is theme-owned and optional**:
  ```bash
  grep -q "MaintenanceBanner.astro" src/layouts/BaseLayout.astro && echo OK || echo "BLOCKING: BaseLayout must optionally load theme MaintenanceBanner.astro"
  grep -q "site?.maintenance?.enabled" src/layouts/BaseLayout.astro && echo OK || echo "BLOCKING: maintenance setting must gate the optional theme banner"
  grep -q "Deze website is tijdelijk in onderhoud" src/layouts/BaseLayout.astro && echo OK || echo "BLOCKING: maintenance banner needs a Dutch fallback message"
  if test -f src/components/MaintenanceBanner.astro; then
    grep -q "message" src/components/MaintenanceBanner.astro && echo OK || echo "BLOCKING: theme MaintenanceBanner must accept/render message"
  else
    echo "OK: theme has no MaintenanceBanner component; no-op is allowed"
  fi
  ```
- **`scripts/build-cms-css.mjs` exists + invoked in package.json build**:
  ...
  ```
- **Responsive canvas checker exists + passes**:
  ```bash
  test -f scripts/check-responsive.mjs && echo OK || echo BLOCKING
  pnpm check:responsive
  ```
- **Docker runtime does not require write access to `/data` for CMS artifact sync**:
  ```bash
...
- **`docker-compose.cms.yml.example` mounts `/data:ro`**:
  ```bash
  grep -q "/data:ro" docker-compose.cms.yml.example && echo OK || echo BLOCKING
  ```
