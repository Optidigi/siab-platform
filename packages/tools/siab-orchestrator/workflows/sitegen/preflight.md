...
file is opened directly, verify it before reading further. Phase 9 needs it for
`gh repo create`, `gh run watch`, and `gh api`. If this fails, stop and tell the
...
The user runs the operational side (VPS compose, Traefik route labels, DNS,
TLS); your scope ends at "image published to ghcr.io and user confirmed deploy".

## The deploy chain end-to-end

```
packages/site-template (boilerplate) ─┐
                                      ├─→ packages/tools/siab-orchestrator/site-<slug>/
packages/site-themes (theme used)   ─┘                  │
                                                         └─→ optidigi/site-<slug> repo
                                                                            │  push to main
                                                                            ↓
