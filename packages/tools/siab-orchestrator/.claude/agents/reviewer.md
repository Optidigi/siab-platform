...
- `siteManifest.json` present at site package root (per-tenant manifest used by
  ...
  - The `blocks[]` array inside the manifest gates the CMS "Add block" menu
    ...
    + valid slugs documented in `packages/site-template/README.md`
    § "`siteManifest.blocks[]` — the per-tenant CMS block menu".
  - The manifest should include the SIAB analytics contract unless analytics
    is intentionally disabled: `analytics.enabled`, `analytics.dashboardVisible`,
    `analytics.consentMode: "required"`, and `analytics.conversionGoals`.
    Public tracking must remain consent-gated; this review only verifies the
    generated site carries the contract forward for later CMS conversion.
